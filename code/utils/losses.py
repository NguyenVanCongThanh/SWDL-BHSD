...
def dice(output_mask, target, smooth=1e-5):
    """
    计算 Dice 系数
    :param output_mask: 模型输出的分割掩码，形状为 [batch_size, depth, height, width]，值为 0 或 1
    :param target: 真实的分割掩码，形状与 output_mask 相同，值为 0 或 1
    :param smooth: 平滑项，避免除零错误
    :return: Dice 系数，值范围 [0, 1]
    """
    # 将 target 转换为浮点型
    target = target.float()

    # 计算交集
    intersect = torch.sum(output_mask * target)

    # 计算 output_mask 和 target 的大小
    output_sum = torch.sum(output_mask)
    target_sum = torch.sum(target)

    # 计算 Dice 系数
    dice_coeff = (2 * intersect + smooth) / (output_sum + target_sum + smooth)
